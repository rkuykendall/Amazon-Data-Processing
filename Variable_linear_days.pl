...
sub getReviews {
  ...
  $past->subtract_duration( DateTime::Duration->new( days => 1 ) );
  $reviews = $dbh->prepare("SELECT Rating, dateReal FROM Reviews WHERE ( ProductID = ? && ( dateReal <= ? AND dateReal >= ? ) ) ORDER BY dateReal ASC");
  $reviews->execute($id, $date, $past);
...
foreach(@products)
{
  ...
  $oneDay = DateTime::Duration->new( days => 1 );
  $date = $parser->parse_datetime('2006-05-20');
  $date->subtract_duration( DateTime::Duration->new( days => $range ) );
